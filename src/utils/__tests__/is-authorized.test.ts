...
test('accepts the raw secret as the authorization header', () => {
  expect(
    isAuthorized({ ...base, authorizationHeader: 'top-secret', tokenParam: null })
  ).toBe(true)
})

test('accepts the secret as the token query param', () => {
  expect(
    isAuthorized({ ...base, authorizationHeader: null, tokenParam: 'top-secret' })
  ).toBe(true)
})
...
test('rejects a missing or wrong credential', () => {
  ...
  expect(
    isAuthorized({ ...base, authorizationHeader: 'Bearer wrong', tokenParam: 'nope' })
  ).toBe(false)
})
